...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate {
    ...
    func loadUpSomeData() {
        if !hasData() {
            if let records = NSArray(contentsOfURL: NSBundle.mainBundle().URLForResource("AcmeHatCo", withExtension: "plist")!) as? [[String:AnyObject]] {
                for record in records {
                    let product = Product(managedObjectContext: self.managedObjectContext)
                    product.returned = record["Returned"] as? Int
                    product.sold = record["Sold"] as? Int
